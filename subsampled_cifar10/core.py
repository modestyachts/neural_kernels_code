...
class TableLogger():
    def append(self, output, logname=None):
        if not hasattr(self, 'keys'):
            self.keys = output.keys()
            print(*(f'{k:>12s}' for k in self.keys))
            if logname is not None:
                with open(logname, 'a') as log:
                    log.write(repr([*(f'{k:>12s}' for k in self.keys)]) + os.linesep)
        filtered = [output[k] for k in self.keys]
        print(*(f'{v:12.4f}' if isinstance(v, np.float) else f'{v:12}' for v in filtered))
        if logname is not None:
            with open(logname, 'a') as log:
                log.write(repr([*(f'{v:12.4f}' if isinstance(v, np.float) else f'{v:12}' for v in filtered)]) + os.linesep)
...
def zca(train, test, min_divisor=1e-8, zca_bias=0.0001, return_weights=False):
...
def run_batches(model, batches, training, optimizer_step=None, optimizer_zero_grad=None, stats=None):
...
def train_epoch(model, train_batches, test_batches, optimizer_step, optimizer_zero_grad, timer, test_time_in_total=True):
    train_stats, train_time = run_batches(model, train_batches, True, optimizer_step, optimizer_zero_grad), timer()
...
def train(model, optimizer, train_batches, test_batches, epochs, 
    ...
    for epoch in range(epochs):
        epoch_stats = train_epoch(model, train_batches, test_batches, optimizer.step, None, timer, test_time_in_total=test_time_in_total) 
